...
- [解説動画：64秒・日英字幕](demo.mp4)
- [投資家向け動画：48秒・日英字幕](investor.mp4)
- [概要PDF：日本語1ページ](overview-ja.pdf)
- [English overview](overview-en.md)
- [実装状況と検証記録](STATUS.md)

動画は実装画面の静止画ダイジェストと説明スライドです。音声は付けていません。サンプル翻訳を使用し、AI製品が自動で作業する録画ではありません。

### 最も簡単な体験

`index.html` をダウンロードしてブラウザで開くと、ブラウザ内保存の体験版が動きます。GitHubのファイル表示画面では実行されません。初期状態は各チーム300pt、翻訳依頼50ptです。
...
ブラウザで `http://127.0.0.1:8765` を開きます。macOSでは `start.command` をターミナルから実行することもできます。
